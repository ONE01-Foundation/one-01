...
4. ~~**Make Forms real**~~ ✅ DONE — "fill out my X form" sets up the right template (licence / passport / NII / generic) as an in-chat fillable card; YOU type your own values (ONE never auto-fills PII), and Prepare saves the completed form as a `kind:"form"` draft in a process (reusing the Drafts review/approve UI). Works from home (creates the process) or inside a unit. ~~Reminders~~ ✅ DONE — "remind me…" (home or in-process) creates a real held reminder (parses a "when"); booking auto-creates a 24h reminder; pending reminders surface atop Updates on Home with a done-toggle + open-process + dismiss. Persisted in `one_reminders`.
5. ~~**Sources actually feed answers**~~ ✅ DONE — a regulated-domain question (licensing / NII / health / gov) is answered FROM the matching official source (the model is told to answer per that source and to say "verify with the source" when unsure), and the answer carries a citation pill ("🚗 per Licensing Authority · gov.il"). `SOURCE_INTENT` + `sourceForText()`. (Role-gated custom source-adding is still a placeholder.)
...
5. ~~**Forms functional**~~ ✅ DONE — in-chat fillable card (you fill your own values) → saves a `kind:"form"` draft in a process. ~~Sources~~ ✅ DONE — regulated-domain answers cite the official source. ~~Reminders~~ ✅ DONE — "remind me…" + booking auto-reminder; pending reminders surface atop Updates (done-toggle / open-process / dismiss); persisted.
